...
const string strMessageMagic = "arroncoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
	...
	// 0-300 and 1081-1281 is 100,000 block reward
    if (nHeight < 89)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            if (tx.IsCoinStake() && currentHeight < 30 && !fTestNet)
            {
                double nNetworkDriftBuffer = nTxValueOut*.02;
                nTxValueOut = nTxValueOut-nNetworkDriftBuffer;
                nStakeReward = nTxValueOut - nTxValueIn;
            }
            if ((tx.IsCoinStake() && currentHeight >= 30) || (tx.IsCoinStake() && fTestNet))
            {
                nStakeReward = nTxValueOut - nTxValueIn;
            }
            nValueIn += nTxValueIn;
    ...
    if (IsProofOfWork())
    {
        int64_t nReward = GetProofOfWorkReward(nFees, pindex->pprev->nHeight + 1);
    ...
    if (IsProofOfStake())
    {
        ...
        {
            // arroncoin: coin stake tx earns reward instead of paying fee
            uint64_t nStakeTime;
...
// arroncoin: total stake time spent in transaction that is accepted by the network, in the unit of coin-days.
// Only those coins meeting minimum age requirement counts. As those
// transactions not in main chain are not currently indexed so we
// might not find out about their coin age. Older transactions are
// guaranteed to be in main chain by sync-checkpoint. This rule is
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // arroncoin: check proof-of-stake block signature
...
// arroncoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "arroncoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0x5c, 0xcf, 0xd2, 0xe5 };
